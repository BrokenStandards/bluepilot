...
type State struct {
	...
	DivergenceTicks        int
	SpeedLimitGuess        SpeedLimitGuess
}
...
func loop(state *State) {
	...
	state.CarContext = ReadCarContext()
	forceRematch := UpdateDivergence(state)

	state.CurrentWay, err = GetCurrentWay(state.CurrentWay, state.NextWays, offline, pos, state.LastPosition, gpsAccuracy, forceRematch, state.CarContext.Curvature)
	logde(errors.Wrap(err, "could not get current way"))

	roadInfoChanged := hasRoadInfoChanged(oldWay, state.CurrentWay.Way)
	if roadInfoChanged {
	...
	// The guess only depends on the matched way and the road graph, so it is
	// recomputed only when the way changes or the offline data reloads.
	if roadInfoChanged || dataReloaded {
		state.SpeedLimitGuess = ComputeSpeedLimitGuess(state.CurrentWay, offline)
	}
	...
	data, err = json.Marshal(state.SpeedLimitGuess)
	logde(errors.Wrap(err, "could not marshal speed limit guess"))
	err = PutParam(MAP_SPEED_LIMIT_GUESS, data)
	logwe(errors.Wrap(err, "could not write speed limit guess"))
...
func main() {
	...
	log.Info().Str("version", VERSION).Msg("Starting main loop with stability improvements and distance smoothing")
	for {
		loop(&state)
	}
}
